...
def __WIFCONTINUED(status):
    return (status) == __W_CONTINUED  # macro
...
def _IO_peekc(_fp):
    return _IO_peekc_unlocked(_fp)  # macro
...
def __bos(ptr):
    return __builtin_object_size(ptr, __USE_FORTIFY_LEVEL > 1)  # macro
...
def __va_arg_pack_len():
    return __builtin_va_arg_pack_len()  # macro


def __WIFEXITED(status):
    return __WTERMSIG(status) == 0  # macro
...
def WTERMSIG(status):
    return __WTERMSIG(__WAIT_INT(status))  # macro
...
def FD_CLR(fd, fdsetp):
    return __FD_CLR(fd, fdsetp)  # macro
...
def __REDIRECT_LDBL(name, proto, alias):
    return __REDIRECT(name, proto, alias)  # macro
...
def htobe16(x):
    return __bswap_16(x)  # macro


def alloca(size):
    return __builtin_alloca(size)  # macro


def be64toh(x):
    return __bswap_64(x)  # macro
...
def __glibc_likely(cond):
    return __builtin_expect((cond), 1)  # macro
...
def __attribute_format_arg__(x):
    return __attribute__((__format_arg__(x)))  # macro


# def __attribute_alloc_size__(params): return __attribute__ ((__alloc_size__ params)) # macro
def WSTOPSIG(status):
    return __WSTOPSIG(__WAIT_INT(status))  # macro
...
def minor(dev):
    return gnu_dev_minor(dev)  # macro
...
def WIFSTOPPED(status):
    return __WIFSTOPPED(__WAIT_INT(status))  # macro
...
def _IO_BE(expr, res):
    return __builtin_expect((expr), res)  # macro
...
def WIFSIGNALED(status):
    return __WIFSIGNALED(__WAIT_INT(status))  # macro


# def __FD_CLR(d,set): return ((void) (__FDS_BITS (set)[__FD_ELT (d)] &= ~__FD_MASK (d))) # macro
def WEXITSTATUS(status):
    return __WEXITSTATUS(__WAIT_INT(status))  # macro
...
def FD_ISSET(fd, fdsetp):
    return __FD_ISSET(fd, fdsetp)  # macro
...
def __GNUC_PREREQ(maj, min):
    return (__GNUC__ << 16) + __GNUC_MINOR__ >= ((maj) << 16) + (min)  # macro


# def __LDBL_REDIR1_NTH(name,proto,alias): return name proto __THROW # macro
def __GLIBC_PREREQ(maj, min):
    return (__GLIBC__ << 16) + __GLIBC_MINOR__ >= ((maj) << 16) + (min)  # macro
...
def __ASMNAME(cname):
    return __ASMNAME2(__USER_LABEL_PREFIX__, cname)  # macro
...
def __WIFSTOPPED(status):
    return ((status) & 0xFF) == 0x7F  # macro


__codecvt_error = 2
# __BLKCNT64_T_TYPE = __SQUAD_TYPE # alias
def __glibc_unlikely(cond):
    return __builtin_expect((cond), 0)  # macro
...
def __warnattr(msg):
    return __attribute__((__warning__(msg)))  # macro
...
def WIFCONTINUED(status):
    return __WIFCONTINUED(__WAIT_INT(status))  # macro


def __WSTOPSIG(status):
    return __WEXITSTATUS(status)  # macro
...
def __LONG_LONG_PAIR(HI, LO):
    return LO, HI  # macro


def __bos0(ptr):
    return __builtin_object_size(ptr, 0)  # macro


def getc(_fp):
    return _IO_getc(_fp)  # macro
...
def makedev(maj, min):
    return gnu_dev_makedev(maj, min)  # macro


def __attribute_format_strfmon__(a, b):
    return __attribute__((__format__(__strfmon__, a, b)))  # macro


def WIFEXITED(status):
    return __WIFEXITED(__WAIT_INT(status))  # macro
...
def __WCOREDUMP(status):
    return (status) & __WCOREFLAG  # macro


def FD_SET(fd, fdsetp):
    return __FD_SET(fd, fdsetp)  # macro


def major(dev):
    return gnu_dev_major(dev)  # macro
...
def __FD_ISSET(d, set):
    return (__FDS_BITS(set)[__FD_ELT(d)] & __FD_MASK(d)) != 0  # macro
...
def __REDIRECT_NTH_LDBL(name, proto, alias):
    return __REDIRECT_NTH(name, proto, alias)  # macro


def __va_arg_pack():
    return __builtin_va_arg_pack()  # macro
...
__obstack_vprintf_chk.argtypes = [
    POINTER(obstack),
    c_int,
    STRING,
    POINTER(__va_list_tag),
...
_IO_2_1_stdin_ = (_IO_FILE_plus).in_dll(
    _libraries["/usr/lib/libmikmod.so"], "_IO_2_1_stdin_"
)
_IO_2_1_stdout_ = (_IO_FILE_plus).in_dll(
    _libraries["/usr/lib/libmikmod.so"], "_IO_2_1_stdout_"
)
_IO_2_1_stderr_ = (_IO_FILE_plus).in_dll(
    _libraries["/usr/lib/libmikmod.so"], "_IO_2_1_stderr_"
)
...
MikMod_RegisterErrorHandler = _libraries[
...
MikMod_RegisterAllDrivers = _libraries[
    "/usr/lib/libmikmod.so"
...
MikMod_RegisterAllLoaders = _libraries[
    "/usr/lib/libmikmod.so"
