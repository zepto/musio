...
_gme_lib = ctypes.cdll.LoadLibrary(gmelib_name)
...
class Music_Emu_t(ctypes.Structure):
    _fields_ = []
...
gme_track_count.argtypes = [ctypes.POINTER(Music_Emu)]
gme_track_count.restype = ctypes.c_int

# Start a track, where 0 is the first track
# gme_err_t gme_start_track( Music_Emu*, int index );
gme_start_track = _gme_lib.gme_start_track
gme_start_track.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_int]
...
gme_delete.argtypes = [ctypes.POINTER(Music_Emu)]
...
gme_set_fade.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_int]
gme_set_fade.restype = None

# True if a track has reached its end
# int gme_track_ended( Music_Emu const* );
gme_track_ended = _gme_lib.gme_track_ended
gme_track_ended.argtypes = [ctypes.POINTER(Music_Emu)]
gme_track_ended.restype = ctypes.c_int

# Number of milliseconds (1000 = one second) played since beginning of track
# int gme_tell( Music_Emu const* );
gme_tell = _gme_lib.gme_tell
gme_tell.argtypes = [ctypes.POINTER(Music_Emu)]
gme_tell.restype = ctypes.c_int

# Seek to new time in track. Seeking backwards or far forward can take a while.
# gme_err_t gme_seek( Music_Emu*, int msec );
gme_seek = _gme_lib.gme_seek
gme_seek.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_int]
...
gme_warning.argtypes = [ctypes.POINTER(Music_Emu)]
gme_warning.restype = ctypes.c_char_p
...
gme_load_m3u.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_char_p]
...
# gme_clear_playlist.argtypes = [ctypes.POINTER(Music_Emu)]
...
gme_free_info.argtypes = [ctypes.POINTER(gme_info_t)]
gme_free_info.restype = None

gme_info_t._fields_ = [
    # times in milliseconds; -1 if unknown
    ("length", ctypes.c_int),  # total length, if file specifies it
    ("intro_length", ctypes.c_int),  # length of song up to looping section
    ("loop_length", ctypes.c_int),  # length of looping section
...
gme_set_stereo_depth.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_double]
...
gme_ignore_silence.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_int]
...
gme_set_tempo.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_double]
gme_set_tempo.restype = None

# Number of voices used by currently loaded file
# int gme_voice_count( Music_Emu const* );
gme_voice_count = _gme_lib.gme_voice_count
gme_voice_count.argtypes = [ctypes.POINTER(Music_Emu)]
gme_voice_count.restype = ctypes.c_int
...
gme_voice_name.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_int]
gme_voice_name.restype = ctypes.c_char_p

# Mute/unmute voice i, where voice 0 is first voice
# void gme_mute_voice( Music_Emu*, int index, int mute );
gme_mute_voice = _gme_lib.gme_mute_voice
gme_mute_voice.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_int, ctypes.c_int]
...
gme_mute_voices.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_int]
...
class gme_equalizer_t(ctypes.Structure):
    _fields_ = [
        ...
        ("bass", ctypes.c_double),  # 1 = full bass, 90 = average,
...
gme_equalizer.argtypes = [ctypes.POINTER(Music_Emu), ctypes.POINTER(gme_equalizer_t)]
...
class gme_type_t_(ctypes.Structure):
    _fields_ = []


gme_type_t = ctypes.POINTER(gme_type_t_)
...
gme_type.argtypes = [ctypes.POINTER(Music_Emu)]
...
# gme_type_multitrack.restype = ctypes.c_int
...
gme_identify_header.argtypes = [ctypes.c_void_p]
gme_identify_header.restype = ctypes.c_char_p
...
gme_identify_extension.argtypes = [ctypes.c_char_p]
...
gme_identify_file.argtypes = [ctypes.c_char_p, ctypes.POINTER(gme_type_t)]
...
gme_new_emu.argtypes = [gme_type_t, ctypes.c_int]
gme_new_emu.restype = ctypes.POINTER(Music_Emu)

# Load music file into emulator
# gme_err_t gme_load_file( Music_Emu*, const char path [] );
gme_load_file = _gme_lib.gme_load_file
gme_load_file.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_char_p]
...
gme_load_data.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_void_p, ctypes.c_long]
...
gme_load_m3u_data.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_void_p, ctypes.c_long]
...
gme_set_user_data.argtypes = [ctypes.POINTER(Music_Emu), ctypes.c_void_p]
gme_set_user_data.restype = None

# void* gme_user_data( Music_Emu const* );
gme_user_data = _gme_lib.gme_user_data
gme_user_data.argtypes = [ctypes.POINTER(Music_Emu)]
gme_user_data.restype = ctypes.c_void_p
...
gme_user_cleanup_t = ctypes.CFUNCTYPE(ctypes.c_void_p, ctypes.c_void_p)

# void gme_set_user_cleanup( Music_Emu*, gme_user_cleanup_t func );
gme_set_user_cleanup = _gme_lib.gme_set_user_cleanup
gme_set_user_cleanup.argtypes = [ctypes.POINTER(Music_Emu), gme_user_cleanup_t]
